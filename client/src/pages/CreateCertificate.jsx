...
function CreateCertificate() {
  ...
  const handleAddSignatureBox = () => {
    if (formData.signatureBoxes.length >= 3) return;
    setFormData((prev) => ({
      ...prev,
      signatureBoxes: [
        ...prev.signatureBoxes,
  ...
  const buildSignaturePayload = () => {
    const cleanedBoxes = cleanSignatureBoxes(formData.signatureBoxes);
  ...
  const previewData = generatedCertificate
    ? {
        ...formData,
        certificateCategory: generatedCertificate.certificateCategory || formData.category,
        certificateId: generatedCertificate.certificateId,
        createdAt: generatedCertificate.createdAt,
        signatureBoxes: generatedCertificate.signatureBoxes || formData.signatureBoxes,
    ...
    <section className="space-y-8 pb-10">
      ...
      <div className="app-glass-hero p-7 lg:p-9 animate-hero-fade-in">
        ...
        <p className="mt-2 max-w-3xl text-base text-slate-600 font-medium leading-relaxed">
          Fill out participant and event information, select an official template design, configure custom signature settings, preview live in real-time, and export high-resolution PDF or PNG.
        </p>
      </div>
      ...
      >
        ...
        <div>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-3 mb-5 gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-semibold">
                  Create up to 3 custom signature blocks for your certificate.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs font-extrabold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200">
                Signature Boxes: <span className="text-slate-900 font-black">{formData.signatureBoxes.length} / 3</span>
              </span>
              <button
                type="button"
                onClick={handleAddSignatureBox}
                disabled={formData.signatureBoxes.length >= 3}
                className="rounded-xl bg-blue-600 px-4 py-2 text-xs font-black text-white hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-xs active:scale-98"
              >
          ...
          {/* Maximum 3 subtle notice */}
          {formData.signatureBoxes.length >= 3 && (
            <div className="mb-4 rounded-xl border border-amber-200/80 bg-amber-50/80 px-4 py-2.5 text-xs font-bold text-amber-800 flex items-center gap-2">
              <span>ℹ️</span>
              <span>Maximum 3 signature boxes allowed.</span>
            </div>
          )}

          {/* Single Signature Position Selector */}
          {formData.signatureBoxes.length === 1 && (
            <div className="mb-5 rounded-2xl border border-blue-100 bg-blue-50/40 p-4 space-y-2">
              ...
              <div className="inline-flex rounded-xl bg-white p-1 border border-slate-200 shadow-xs">
                {["left", "center", "right"].map((pos) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setFormData((prev) => ({ ...prev, singleSignaturePosition: pos }));
                      setGeneratedCertificate(null);
                    }}
                    className={`rounded-lg px-4 py-2 text-xs font-extrabold capitalize transition ${
                      (formData.singleSignaturePosition || "center") === pos
          ...
          {/* Signature Boxes List or Empty State */}
          {formData.signatureBoxes.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/50 p-8 text-center space-y-1">
              <p className="text-sm font-bold text-slate-700">No signature boxes added.</p>
              <p className="text-xs text-slate-500 font-medium max-w-md mx-auto">
                Add up to 3 signature blocks for chairs, convenors, coordinators, or other authorized persons.
              </p>
            </div>
          ) : (
            <div className="grid gap-5 md:grid-cols-2">
              {formData.signatureBoxes.map((box, index) => (
                <SignatureBoxEditor
                  key={index}
                  box={box}
                  index={index}
